...
export default function LoginPage({ onRegister }: Props) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      if (newFail >= 5) { setCooldown(30); setFailCount(0); setErr('Terlalu banyak percobaan. Tunggu 30 detik.') }
      else setErr(error)
      triggerShake()
    }
  }
    ...
    <div style={{
      ...
      >
        ...
        <form onSubmit={handleLogin} autoComplete="on" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div>
            <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--silver3)', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 6, fontFamily: 'var(--mono)' }}>
              Username
            </label>
            <input
              ...
              style={{
                ...
                background: 'var(--bg4)', border: `1px solid ${err && !username ? 'var(--red)' : 'var(--border2)'}`,
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{
                  ...
                  background: 'var(--bg4)', border: `1px solid ${err && !password ? 'var(--red)' : 'var(--border2)'}`,
                  borderRadius: 10, fontSize: 14, color: 'var(--silver)',
                  fontFamily: 'var(--font)', outline: 'none', boxSizing: 'border-box',
                  transition: 'border-color 150ms',
                }}
          ...
          >
            {loading ? (
              <>
                <span style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white', borderRadius: '50%', animation: 'loginSpin 0.7s linear infinite', display: 'inline-block' }} />
                Masuk...
              </>
        ...
        <div style={{ marginTop: 24, textAlign: 'center', fontSize: 13, color: 'var(--silver3)' }}>
          Belum punya akun?{' '}
          <span onClick={onRegister} style={{ color: 'var(--accent)', fontWeight: 600, cursor: 'pointer' }}>
            Daftar di sini
          </span>
        </div>
      </div>
