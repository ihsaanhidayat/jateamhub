...
interface ChatState {
  ...
  // Realtime — owned by App.tsx, not ChatPage
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  loadConversations: async (userId) => {
    ...
    // Set unreadTotal from DB snapshot
  ...
  selectConv: async (id, userId) => {
    ...
    // Only mark read if the tab is actually visible
  ...
  startConv: async (createdBy, participantB) => {
    // Correct dedup: both participants must match exactly
  ...
  sendText: async (text, senderId) => {
    ...
    if (msg) {
      set(s => ({
        // Deduplicate by id in case realtime fires before this
  ...
  sendFile: async (file, senderId) => {
    ...
    const type = file.type.startsWith('image/') ? 'image'
               ...
               : 'document'
    const msg = await sendMessage(convId, senderId, null,
      type as ChatMessage['message_type'], result.url, result.name, result.size)
    if (msg) {
      set(s => ({
        messages: s.messages.some(m => m.id === msg.id)
          ? s.messages
  ...
  subscribeAll: (userId) => {
    ...
    const unsub = () => { if (_channel) { _channel.unsubscribe(); _channel = null } }
  ...
  verifyAndUnlock: async (pin) => {
    ...
    // If hasPinSet is true but storage cleared, force re-setup instead of bypassing
    if (!hash || !salt) {
...
// Lock on logout — registered once at module level
